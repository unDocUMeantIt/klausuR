...
#' @param body_vars Optional R object to be added to the available objects in the RMarkdown body. If you provide this, you can use it in the RMarkdown body of custom templates.
#'    If missing, the respective value of \code{meta} will be used by default.
...
#' @param extra_blank_page Logical, if \code{TRUE} and \code{merge = TRUE}, a blank page will be added after each report. This can be useful if the reports
#'    have an odd number of pages but are supposed to be printed duplex.
#' @param quiet Logical, if \code{TRUE} no feedback messages on the current status are given.
#' @param cores Number of CPU cores to use for rendering individual PDFs in parallel.
...
setMethod(
  ...
  function(
    ...
    if(missing(body_vars)){
      body_vars <- meta[["body_vars"]]
    } else {}
    ...
    klsr_mean     <- slot(klsr, "mean")
    klsr_sd       <- slot(klsr, "sd")
    klsr_cronbach <- slot(klsr, "cronbach")
    klsr_max      <- length(slot(klsr, "marks"))
    ...
    if(identical(matn, "glob")){
      ...
      pre_body <- paste(
        ...
        , paste(
              "colnames_global <- c("
            , paste0("    No = \"",        ifelse(is.null(header[["labels"]][["testno"]]),     "No",         header[["labels"]][["testno"]]), "\"")
            , paste0("  , Name = \"",      ifelse(is.null(header[["labels"]][["familyname"]]), "Name",       header[["labels"]][["familyname"]]), "\"")
            , paste0("  , FirstName = \"", ifelse(is.null(header[["labels"]][["firstname"]]),  "FirstName",  header[["labels"]][["firstname"]]), "\"")
            , paste0("  , MatrNo = \"",    ifelse(is.null(header[["labels"]][["matno_abbr"]]), "MatrNo",     header[["labels"]][["matno_abbr"]]), "\"")
            , paste0("  , Points = \"",    ifelse(is.null(header[["labels"]][["points"]]),     "Points",     header[["labels"]][["points"]]), "\"")
            , paste0("  , Percent = \"",   ifelse(is.null(header[["labels"]][["percent"]]),    "Percent",    header[["labels"]][["percent"]]), "\"")
            , paste0("  , Mark = \"",      ifelse(is.null(header[["labels"]][["mark"]]),       "Mark",       header[["labels"]][["mark"]]), "\"")
            , paste0("  , Pseudonym = \"", ifelse(is.null(header[["labels"]][["pseudonym"]]),  "Pseudonym",  header[["labels"]][["pseudonym"]]), "\"")
            , ")"
            , sep="\n"
          )
          ...
          , paste0("test_mean <- ", paste0(deparse(klsr_mean), collapse="\n"), collapse="\n")
          , paste0("test_sd <- ", paste0(deparse(klsr_sd), collapse="\n"), collapse="\n")
          , paste0("test_cronbach <- ", paste0(deparse(klsr_cronbach), collapse="\n"), collapse="\n")
          , paste0("test_max <- ", klsr_max, collapse="\n")
          , paste0("statistics <- ", paste0(deparse(statistics), collapse="\n"), collapse="\n")
          , paste0("body_vars <- ", paste0(deparse(body_vars), collapse="\n"), collapse="\n")
        , "```\n\n"
        , sep = "\n"
      )
          ...
          parallel::parSapply(
            ...
            , function(this_matn){
                report(
                  ...
                  , sort_by = sort_by
                  , merge = merge
                  , extra_blank_page = extra_blank_page
                  , quiet = quiet
                  , cores = 1
                  , meta = meta
